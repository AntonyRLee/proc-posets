...
def test_totally_parallel_reduces_to_the_bhattacharyya_angle():
    # The identity the thesis names when it says the suite pins each paper's closed forms:
    # a totally parallel model imposes no order, so it IS its activity set, and the block
    # distance between two such mixtures collapses onto the angle between the mixtures'
    # activity-set distributions.
    P_ab = par(leaf("a"), leaf("b"))
    P_ac = par(leaf("a"), leaf("c"))
    P_abc = par(leaf("a"), leaf("b"), leaf("c"))
    for m, n in [
        ([(P_ab, 0.7), (P_ac, 0.3)], [(P_ab, 0.4), (P_abc, 0.6)]),
        ([(P_ab, 1.0)], [(P_abc, 1.0)]),
        ([(P_ab, 0.5), (P_abc, 0.5)], [(P_ac, 0.5), (P_abc, 0.5)]),
    ]:
        assert math.isclose(smd(m, n)[0], bhattacharyya_angle(m, n), rel_tol=1e-12)
