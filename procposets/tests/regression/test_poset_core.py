...
# ---------------------------------------------------------------------------
# audited-defect guards (BACKLOG W12.1, W12.4, W18, W21)
# ---------------------------------------------------------------------------
...
def test_ideal_dp_refuses_wide_posets():
    ...
    wide = frozenset(f"x{i:02d}" for i in range(30))
    # NOT an antichain: a disjoint union of chains has a closed form and is
    # counted instantly (see disjoint_chain_lengths), so the shape that
    # exercises the guard must have a non-chain component.  One V does it.
    v_plus_singletons = frozenset({("x00", "x01"), ("x00", "x02")})
    with pytest.raises(IdealBudgetExceeded):
        count_linear_extensions(wide, v_plus_singletons)
    with pytest.raises(IdealBudgetExceeded):
        sample_linear_extension(wide, v_plus_singletons, random.Random(0))
    # the antichain itself is now free rather than refused -- e = m!
    assert count_linear_extensions(wide, frozenset()) == math.factorial(30)
    # ...but SAMPLING still walks the ideal lattice, so it still refuses
    with pytest.raises(IdealBudgetExceeded):
