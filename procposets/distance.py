"""Information-geometry distances between process models.

Both are built from the Bhattacharyya coefficient ``BC(p, q) = sum_x sqrt(p(x) q(x))``, whose arccos
is the Fisher-Rao geodesic distance on the categorical simplex. They differ in what they apply it to:

``smd``
    The stochastic-matrix distance: the root-mean-square of the per-row Bhattacharyya angle over the
    rows of two block-transition matrices. Compares Markov *transitions*, so it is
    concurrency-preserving and graded -- models that differ a little score a little.
``bhattacharyya_angle``
    The angle on the flat normal-form distribution. Compares Markov *states* (a single vector), and
    is brittle: any two distinct normal forms sit at the maximum distance from one another, with no
    gradation between them.

Evaluating both on the same pair is usually the informative move, since the gap between them
localises whether a difference is one of state or one of dynamics.

The matrix construction follows Lee, *A distance function for stochastic matrices*.
"""
...
#   "selfloop" (option 1) -- unused state s -> s; END -> END (absorbing).
...
def smd_rows(built1, built2, mode: Optional[Mode] = None, normalize: bool = False,
    ...
    `normalize` divides by sqrt(|X|), which turns the sum over rows into a mean:
    ...
    and breaks the triangle inequality. `states` must be a
...
def smd(model1: Model, model2: Model, mode: Optional[Mode] = None, context_depth: int = 1,
        normalize: bool = False, *, context: str = "uniform",
        ceiling: Optional[int] = None) -> tuple[float, dict[str, float]]:
    ...
    ("sink" default, or "selfloop"); see NORMALISATION above. `context_depth` is the fixed-ORDER
    dial passed to `build` (1 = memoryless blocks; higher = sharper, less row-sharing). `normalize`
    applies the sqrt(|X|) factor for cross-state-space-size comparability (see
    ...
    `context="canonical"` compares on the lossless context tree instead, built over BOTH models so
    they share a state space (the pair is the fleet). `ceiling` caps context length there; None is
    the parameter-free lossless chart. `context_depth` is ignored in that mode."""
    if context == "canonical":
        from .context import context_tree
        tree = context_tree([model1, model2], ceiling=ceiling)
        return smd_rows(build(model1, tree=tree), build(model2, tree=tree), mode, normalize)
...
def smd_pairwise(models: list[Model], mode: Optional[Mode] = None, context_depth: int = 1,
                 normalize: bool = False, *, context: str = "uniform",
                 ceiling: Optional[int] = None) -> list[list[float]]:
    ...
    fleet-wide constant |X|). This is the correct normalised fleet
    ...
    `context="canonical"` charts every model on ONE fleet-wide lossless context tree -- a context
    is merged only where it is safe in every member -- which is the construction that keeps |X|
    fleet-wide by design rather than by convention. `ceiling` caps context length; None is the
    parameter-free lossless chart."""
    if context == "canonical":
        from .context import context_tree
        tree = context_tree(models, ceiling=ceiling)
        built = [build(m, tree=tree) for m in models]
        states = sorted(set().union(*(s for _, s in built)))
        return _pairwise_rows([_augment(m, states, mode) for m, _ in built], states, normalize)
...
def bhattacharyya_angle(model1: Model, model2: Model) -> float:
    """Bhattacharyya angle between the flat normal-form distributions."""
