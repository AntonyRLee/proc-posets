"""Loops: geometric unrolling with truncation.

A loop is the one structure the causal-poset pipeline cannot hold as a finite poset: its
longest tiling word L is unbounded, so the finite-depth faithfulness bound fails and full fidelity
would need an infinite state space. The resolution taken here: the loop enters the
...
def empirical_loop_model(body: Poset, repeats: list[int], pre: tuple = (),
    ...
    data-driven cut-off, with no K to choose."""
