...
def _expand(ports: list[Port], g: Generator) -> list[Port]:
    """One slot per unit of object-count: a weight-*k* leg repeated *k* times.

    Order-preserving, so callers sort first and get the copies of a leg sitting
    next to each other. Weight defaults to 1 (:meth:`Generator.weight`), so an
    ungrounded generator round-trips unchanged."""
    return [p for p in ports for _ in range(g.weight(p))]
...
def _box_sub(g: Generator, style: LayoutStyle) -> _Sub:
    ...
    # A boundary is a *multiset*: under a grounding, a leg of weight k carries k
    # typed wires, so it must occupy k port slots. `g.left`/`g.right` are sets
    # and cannot express that, so expand each port to `g.weight(p)` copies here
    # -- kept adjacent (expansion is after the sort) so the k slots read as one
    # bundle. Without this a weight-k leg gets a single slot, `_seq` can pair
    # only one of its k producers, and the surplus producer endpoints fall
    # through `_finish` as *boundary stubs*: wires dangling in mid-air that look
    # like open legs but whose partner slot was simply never built.
    ins = _expand(ins, g)
    outs = _expand(outs, g)
...
def lower_best(subs: list["_Sub"], style: LayoutStyle) -> Layout:
    """Lower each candidate ``_Sub`` with :func:`lower_term` and return the
    fewest-crossing drawing, tie-broken toward the straightest -- the same
    preference :func:`lower_term` applies internally across its straighten rounds,
    lifted one level up to choose across whole candidate *layouts*.

    The candidates must draw the **same morphism** (alternative faithful column
    layerings of one pomset, e.g. :func:`._relayer.relayered_terms` plus the
    as-composed term): they differ only in box columns, so picking the least
    tangled one is purely a readability choice.  With a single candidate this is
    just :func:`lower_term`.  Deterministic; ``subs`` must be non-empty."""
    best: Layout | None = None
    for sub in subs:
        lay = lower_term(sub, style)
        if best is None or (_count_crossings(lay), -_straight_count(lay)) < \
                           (_count_crossings(best), -_straight_count(best)):
            best = lay
    assert best is not None, "lower_best requires at least one candidate"
    return best
