...
def render(
    ...
    *,
    ...
    if isinstance(obj, (list, tuple)):
        # candidate faithful layerings of ONE morphism (e.g. as-composed + the
        # re-layered ASAP/ALAP terms from :mod:`._relayer`): lower each and draw
        # the least-tangled. Never worse than any single candidate on its own.
        if not obj or not all(isinstance(o, Diagram) for o in obj):
            raise TypeError("render() list argument must be non-empty Diagrams")
        subs = [o._sub(lay) for o in obj]
        if offset is not None:
            subs = [s.shift(*offset) for s in subs]
        layout = lower_best(subs, lay)
    ...
    # graphical N-linear bindings: conservation drawn as a coupling arc over the box
...
def _binding_graphics(g) -> tuple[list, list]:
    """The generator's multi-leg relations split for graphical rendering:
...
def _leg_card_by_port(g) -> dict:
    """Per-leg binding cardinality keyed by :class:`~procposets.cospan.signature.Port`,
