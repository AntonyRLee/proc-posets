"""OCCN faithful per-firing grounding -- the OCCN half of unrolling.
...
def _firing_system(t: str, ig: MarkerGroup, og: MarkerGroup) -> frozenset:
    """Per-firing constraint system for context ``(ig, og)``: every marker's
...
def ground_occn(occn: OCCN, *, order: int = 8, max_assignments: int = 200_000) -> Signature:
    """Mined OCCN -> grounded (weighted) signature with per-type
...
def gamma_boundary(occn: OCCN, counts: dict[str, int], *, order: int) -> set[Generator]:
    """The γ1/γ2 boundary of one *run*: γ1 and γ2 are the boundary *objects* of
...
def ground_run(occn: OCCN, counts: dict[str, int], *, order: int = 8, max_assignments: int = 200_000) -> Signature:
    """Grounded signature for one run with per-type object content ``counts``:
