"""Phase 2: marker-group discovery, following Listing 1.1 of Liss et al. (CAiSE 2025).

This implements the *published* pseudocode (per-activity markers from the
...
Pipeline (per Liss et al.):
  ...
  3. keys (their Listing 1.1): markers of the SAME type whose object sets are disjoint
     ...
     no same-type split ever arises, so all keys are unique.]
...
# ---------------------------------------------------------------------------
# per-event marker group + key unification
# ---------------------------------------------------------------------------
