...
Construction of the OCCN -> cospan translation:
...
**Bindings surfaced as leg-multiplicity constraints.** A ``Port`` is a bare typed triple (the
...
def _leg_constraints(t: str, ig: MarkerGroup, og: MarkerGroup) -> frozenset:
    """Surface the markers' bindings as linear constraints on the legs --
    the generator's full **per-leg N-linear blueprint**:
    ...
    *different* generators. They are only reconciled when the cospans are composed --
    ...
    multiplicity) *at composition/grounding*, not pre-resolved here. Earlier this
    ...
    ``signature.json``/``cospans.svg``/the comparison. The behavioural splice
...
def occn_to_signature(occn: OCCN, *, bindings: bool = True) -> Signature:
    ...
    ``bindings`` (default ``True``) surfaces the OCCN's multi-object machinery as
