...
Kept here so there is exactly one
...
def disjoint_chain_lengths(elements, pairs) -> "List[int] | None":
    """Chain lengths if the order is a disjoint union of chains, else ``None``.

    A *disjoint union of chains* is the one shape whose extension count has a
    closed form: interleaving chains of lengths ``c_1..c_k`` gives the
    multinomial ``(sum c_i)! / prod(c_i!)``, so no DP is needed and no budget
    can be exceeded.

    Two cases that matter reduce to it.  The **empty order** is all singleton
    chains, so it counts as ``m!`` -- it is always a meet-closure candidate and
    was previously *refused* at ``m >= 20`` despite its count being free.  And
    the **occurrence chains** of a loop-indexed log (``a@1 < a@2 < ...``, one
    chain per activity) are exactly this shape, which is what makes the
    ``eps`` base measure ``|L(C)|`` cheap to compute exactly.

    Detection: union the comparability classes, then a class of size ``k`` is a
    chain iff it carries exactly ``k(k-1)/2`` ordered pairs (transitively
    closed, so a total order on ``k`` elements has that many and anything
    weaker has fewer).
    """
    items = list(elements)
    uf = UnionFind(items)
    ...
    for a, b in pairs:
        uf.union(a, b)
    for a, b in pairs:
        per_class[uf.find(a)] = per_class.get(uf.find(a), 0) + 1
    lengths = []
    for members in uf.groups():
        k = len(members)
        if per_class.get(uf.find(members[0]), 0) != k * (k - 1) // 2:
            return None                      # this class is not a total order
        lengths.append(k)
    return lengths
...
def count_extensions(elements, pairs, max_states: int = MAX_IDEAL_STATES) -> int:
    ...
    A disjoint union of chains takes the closed form instead (see
    :func:`disjoint_chain_lengths`) -- exact, instant, and never budget-bound,
    which is what stops the empty order being refused at large ``m``.
    """
    chains = disjoint_chain_lengths(elements, pairs)
    if chains is not None:
        total = factorial(sum(chains))
        for c in chains:
            total //= factorial(c)
        return total
