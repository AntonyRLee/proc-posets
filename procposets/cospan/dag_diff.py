...
false agreement on, e.g., an N-poset vs a complete bipartite order),
this module compares the **boundary-rooted labelled DAG** (the label-pomset)
up to isomorphism -- a genuine poset comparison.
...
by the certificate ``(anchor_types, body-DAG)`` so a loop's splice point
...
class DagClass:
    ...
    Loop anchor-type signature (``()`` for closing instances, whose
