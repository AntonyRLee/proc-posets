...
class Port:
    ...
    the generator, keyed by port, not as ``Port`` fields."""
...
class LinearConstraint:
    """A linear (in)equality over a generator's leg-multiplicity variables.
...
class Generator:
    ...
    ``constraints`` is the optional leg-multiplicity decoration -- general linear inequalities
    ...
    ``weights`` is the optional **grounding**: a boundary is a *multiset* of typed
    ...
    def weight(self, port: Port) -> int:
        """Object-count on boundary leg ``port`` under grounding; 1 if unset."""
