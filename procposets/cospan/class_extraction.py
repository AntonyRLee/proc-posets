...
def _left_weighted(g: Generator) -> Counter[Port]:
    """Left boundary as a weighted multiset under grounding: leg ``p`` carries ``g.weight(p)``
...
def _merge_parallel(frontiers_before: list[Counter[Port]], moves: list[Generator]) -> list:
    ...
    their own copy of activity ``a``) must NOT be merged into one group, or
...
# concurrent generators -- has been removed in favour of the categorical
