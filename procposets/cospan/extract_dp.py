...
``occurrence.canonical_key``. So the right unit of
...
* **State** = ``(frontier, used_labels)`` -- the multiset of pending typed
  ...
  already fired. **One origin per label**: same-label zero-left
  ...
  process instance per closing. The Markov property holds on the state.
...
def extract_classes(
    ...
    """Pomset-DP extraction: the iso-class-quotiented catalogue.
    ...
    (e.g. an OCPN of independent per-type subnets) hits the cap, which
    ...
    # ``END_``). gamma2 drains are zero-right -> not single-firing-constrained (the rule is gamma1
    ...
    def closing_pomsets(state: State) -> list[tuple]:
        ...
        for g, ns in graph.succ.get(state, []):
            ...
            for (_, _, sub_body) in closing_pomsets(ns):
                ...
                reps.append(entry)
                if len(reps) >= max_pomsets_per_state:  # over-generation guard: cap the
