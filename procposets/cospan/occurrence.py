...
while still encoding causality -- a real poset comparison, not the
lossy label-skeleton string comparison.
...
not represented as a back-edge here.

Reading convention: every edge is **conjunctive (AND)**. At each event
...
IN = "__IN__"
OUT = "__OUT__"
# The single boundary nodes of a *closing* pomset: all of a notation's origin
...
# connected γ1 source / γ2 sink. This is a render/canonical-key concern only; it does
...
def to_event_dag(
    ...
    *,
    ...
    the comparison needs to keep (it is what distinguishes an N-poset from a complete
    bipartite order).

    ``strip_prefixes`` names boundary-wrapper activities (default the OCCN
    ``START_<ot>``/``END_<ot>`` markers). These are not merely
    ...
    master/OCPN's by that one edge, which was what kept the
    ...
    # Origin/termination markers map onto the single γ1 source / γ2 sink. A loop's
    ...
    for frame in frames:
        ...
        for gen in frame:
            ...
            if role is None:
                ...
                node_id += 1
            # weight-aware: a leg of multiplicity w wires w tokens, so a
...
def is_isomorphic(a: EventDag, b: EventDag) -> bool:
    ...
    the comparison *boundary-rooted*: admissible isos fix the
    boundary."""
...
def history_keys(dag: EventDag) -> dict:
    ...
    occurrence-net union / prefix-trie) collapses shared prefixes
...
def anchor_types(nm: NamedMorphism) -> tuple:
    """The loop anchor projected to a type-multiset: the sorted
