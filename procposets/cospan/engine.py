"""LM-graph -> generator-cospan signature ``Sigma``.

The canonical presentation is built by a four-step pipeline:
...
# explicit terminus activity (the single-boundary convention). There is deliberately no
...
def _traverse(
    ...
    *,
    ...
    if not nbrs:
        ...
        # distinct labels that co-fire one-per-label; minting a same-label ``gamma1``
