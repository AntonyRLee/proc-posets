...
generating grammar of the model's trace language**: a run γ1→γ2 is a
...
* **Algebraic** (:class:`AlgebraicTerm`) -- the boundary-stripped step sequence
  ...
  causal pomset for series-parallel families.
...
``to_dict`` is byte-stable across runs and hash seeds.
"""
...
class Pomset:
    ...
    def from_event_dag(dag) -> "Pomset":
        g = dag.graph
        # node ids mix int (real events) and str (the gamma1/gamma2 boundary), so
...
class Family:
    ...
    must use ``pomset`` for exact linearization."""
    ...
    sp_exact: bool
    pomset: Pomset
    constraints: tuple = ()  # the family's accumulated leg-multiplicity system
...
class SpliceRepresentation:
    ...
    def from_extraction_result(
        ...
        """``prune_bound``: if set, drop families whose accumulated leg
        ...
        for spine, members in families_by_spine.items():
            ...
            # the family's leg-multiplicity system = the union of its baseline
...
def _spine_step_splices(
    ...
    coordinates. Two mechanisms unioned (the committed-vs-neutral split, at
