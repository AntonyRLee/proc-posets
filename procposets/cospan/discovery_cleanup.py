...
def forget_provenance(sig: Signature) -> Signature:
    ...
    *fungible* place tokens the future depends only on ``(typ,tgt)``, never on
    ...
    Preserving every closing requires carrying leg **multiplicity** through the
    merge: collapsing two same-``(typ,tgt)`` legs of one boundary into a
    frozenset would drop one, so the collapsed leg records its count as a grounding
    weight (see :func:`side_weights` below). Without that the quotient is not
    behaviour-preserving at all -- it silently reduces what a generator
    consumes.

    Applied as a comparison-time transform over the fine-grained extracted
    ...
    def side_weights(g: Generator, side: frozenset) -> dict:
        """Leg multiplicity on one boundary side after the quotient.

        A boundary is a *multiset* but ``Generator.left``/``right`` are
        frozensets, so multiplicity above 1 can only be carried by grounding
        ``weights``. The quotient is deliberately non-injective, so whenever it
        maps two distinct ports of one side onto the same quotient port the
        collapsed leg MUST record how many legs it stands for -- otherwise the
        generator silently consumes (or produces) fewer objects than it did
        before, which is not a quotient of its behaviour but a change to it.

        Concretely, before this was recorded: a mined OCPN's ``send package``
        consumes ``create package|items`` and ``pick item|items``; both quotient
        to ``*|items|send package`` and the frozenset kept one. Both producers
        still fired, the surplus token was never consumed, the frontier could
        never empty, and every closing vanished (4 -> 0 on order-management).

        Ungrounded legs count 1 each (``Generator.weight`` returns 1 when unset),
        so this also subsumes the previous grounded-only aggregation.
        """
        agg: dict[Port, int] = {}
        for p in side:
            q = quotient(p)
            agg[q] = agg.get(q, 0) + g.weight(p)
        return agg

    def quotient_weights(g: Generator) -> frozenset:
        left, right = side_weights(g, g.left), side_weights(g, g.right)
        both = set(left) & set(right)
        clash = {q for q in both if left[q] != right[q]}
        if clash:
            # `weights` is keyed by port for the whole generator, so one port
            # cannot carry a different multiplicity on each side. Only a
            # self-loop leg can land here. Fail loudly rather than pick a side.
            raise ValueError(
                f"forget_provenance: generator {g.label!r} needs different leg "
                f"multiplicities on its two sides for {sorted(map(str, clash))}; "
                "Generator.weights cannot express that"
            )
        merged = {**left, **right}
        # Weight 1 is the default, so only record what differs -- keeps the
        # quotient a genuine no-op on a provenance-clean signature.
        return frozenset((q, w) for q, w in merged.items() if w != 1)
    ...
    gens = {
        Generator(
            ...
            # carry the leg-multiplicity constraints, remapping their ports through the
...
def gamma_normalize(sig: Signature, *, gamma1: str = "gamma1", gamma2: str = "gamma2") -> Signature:
    ...
    1. **Explicit start/end generators** (OCCN's ``START_<ot>``/``END_<ot>``) are
...
def close_gamma2_termini(sig: Signature, *, gamma2: str = "gamma2") -> Signature:
    ...
    that co-fire one-per-label, whereas a ``gamma2`` drain is zero-*right* and so is
