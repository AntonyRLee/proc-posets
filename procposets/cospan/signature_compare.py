...
this keeps the **generator inventory with its leg-multiplicity parameters
...
and under that key we compare the per-``(side, type)`` binding intervals (the
...
differs by adapter convention (master's single γ1/γ2 interface vs OCCN's
per-type START/END) -- a real, documented structural difference the matrix
...
class BindingProfile:
    """The N-linear leg-multiplicity parameters of a generator, keyed by ``(side, type)`` so it is
...
def binding_profile(g: Generator) -> BindingProfile:
    """Extract :class:`BindingProfile` from one generator's leg-multiplicity constraints.
...
def compare(
    ...
    * ``"joint"`` (default) -- whole generators by :class:`CanonKey`, with
      :class:`BindingProfile` payloads.  The 0-drift oracles pin these
