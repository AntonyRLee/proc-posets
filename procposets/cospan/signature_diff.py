...
2. a within-signature dedup pre-pass reusing ``morphism_schema.schema_classes`` (one
...
degenerate miner noise, or genuine structural disagreement)
...
``START_<ot>``/``END_<ot>`` boundary-generator convention) is
...
def deduped_skeletons(
    ...
    """The ``morphism_schema`` within-signature dedup, applied to one subset of a signature's
...
def decompose_loops(loop_legend: dict[Skeleton, str]) -> tuple[dict[str, tuple], dict[Skeleton, _NameHolder]]:
    ...
    *other, shorter* loops already in the same legend, via
    ...
    this purpose, but it was measured never to fire on a real signature at
...
def strip_boundary_wrapper(skeleton: Skeleton) -> Skeleton:
    ...
    the boundary-generator convention), but have no counterpart in an adapter whose own zero-left/
