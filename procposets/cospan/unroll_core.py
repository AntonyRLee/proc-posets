"""Bounded grounding of a constrained signature -- the notation-agnostic
core.
...
def unroll_generator(g: Generator, *, order: int = 8, max_assignments: int = 200_000) -> set[Generator]:
    """Ground one generator into its weighted concrete instances.
...
def unroll_signature(sig: Signature, *, order: int = 8, max_assignments: int = 200_000) -> Signature:
    ...
    groundings never collapse while symmetric ones never duplicate (the
