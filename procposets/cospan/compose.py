...
string diagrams -- the finite set of "ground truth" diagrams that
``F(Sigma)`` reconstructs (each one a distinct XOR-resolved branch;
...
class _AtomicReady:
    ...
    byte-for-byte -- including grounding weights)."""

    def __init__(self, gens: list[Generator]):
        self.gens = gens

    def ready(self, available: dict[Port, int]) -> list[Generator]:
        # Readiness/consumption respect leg weights under grounding: a grounded
