...
Kemeny family (compare the ORDER RELATIONS) -- TWO charts, on opposite sides of the boundary
--------------------------------------------------------------------------------------------
...
- ``footprint(model)`` / ``footprint_angle(m1, m2)``: the ACTIVITY-LEVEL FOOTPRINT, van der Aalst's
  alpha-relations. Each unordered label pair {x,y} gets a categorical over {x<y, y<x, x||y, x#y}
  read off each variant's POSET and weighted across variants; the distance is the Bhattacharyya
  angle summed over pairs, in the SAME form and units as the block-SMD (Eq. smd). Its ``par``
  outcome is genuine concurrency, so it SEPARATES a concurrency from a coin-flip between the two
  orders: it does NOT factor through the stochastic language, and it is computable only from an
  order-observing channel (never from a flat log).
- ``trace_precedence(model)`` / ``trace_precedence_angle(m1, m2)``: the paper's PAIRWISE CHART
  r_Pi = Prec o r_T. Each unordered pair gets a categorical over the four relations it stands in
  within a SAMPLED TRACE -- {x before y, y before x, interleaved, not both present} -- so it is a
  stochastic-map coarsening of the trace distribution and ties the concurrency to the coin flip.
  This is the chart Kemeny's metric and behavioural profiles are readings of.
  On single total orders the two coincide, and both equal pi*sqrt(K/2) with K the Kemeny distance,
  mirroring the block-SMD's pi*sqrt(H) on deterministic block models.
...
------------------------------------------
Prime blocks fan out over covering-relation atoms and parallel blocks over typed element atoms,
both as uniform splits, leaving the SMD itself unchanged. Everything else in this package stays
atomic, which is the default comparison object. See the comment block above ``disc_angle`` for the conventions,
...
FOOTPRINT_KEYS = ("lt", "gt", "par", "excl")
TRACE_PRECEDENCE_KEYS = ("lt", "gt", "ilv", "excl")


def _pair_relation(P: Poset, x: str, y: str, inv=None) -> str:
    """'lt' if x<y, 'gt' if y<x, 'excl' if the poset omits either, else 'par' (incomparable).

    By label, in a distinct-label poset.  ``inv`` (label->element) may be supplied to skip
    rebuilding it per call -- it depends only on ``P``, so a caller looping over many (x, y)
    hoists it once.  A variant that omits one of the two activities stands in the alpha-
    relation '#' (exclusion) on that pair; before this outcome existed the lookup raised."""
    if inv is None:
        inv = {lab: e for e, lab in P.labels.items()}
    ex, ey = inv.get(x), inv.get(y)
...
def footprint(model):
    """For each unordered label pair, the rho-weighted categorical over {lt, gt, par, excl}.

    The activity-level footprint (alpha-relations), read off each variant's POSET: ``par`` is
    genuine concurrency, so this does not factor through the trace distribution.  See
    :func:`trace_precedence` for the paper's r_Pi, which does."""
    V = alphabet(model)
    ...
    for P, _ in model:
        inv = {lab: e for e, lab in P.labels.items()}
        if len(inv) != len(P.labels):
            raise ValueError("footprint needs distinct labels within a variant; a repeated label "
                             "has no single pairwise relation. Use trace_precedence, which reads "
                             "the trace and records the repeat as 'ilv'.")
        invs.append(inv)
    out = {}
    for x, y in combinations(V, 2):
        cat = dict.fromkeys(FOOTPRINT_KEYS, 0.0)
...
def footprint_angle(m1, m2):
    """Bhattacharyya angle between the two models' activity-level footprints (the 'Kemeny angle').
    Same form as the block-SMD, over the common label pairs. Requires a common alphabet."""
    return _pair_chart_angle(footprint(m1), footprint(m2), FOOTPRINT_KEYS,
                             alphabet(m1), alphabet(m2), "footprint_angle")


def _trace_pair_relation(word, x: str, y: str) -> str:
    """The relation the pair {x, y} stands in within one trace: 'lt' (every occurrence order
    puts x first), 'gt', 'ilv' (both weak orders hold, which a repeated label allows), or
    'excl' (the trace omits one of them)."""
    xs = [i for i, lab in enumerate(word) if lab == x]
    ys = [i for i, lab in enumerate(word) if lab == y]
    if not xs or not ys:
        return "excl"
    x_before_y = xs[0] < ys[-1]
    y_before_x = ys[0] < xs[-1]
    if x_before_y and y_before_x:
        return "ilv"
    return "lt" if x_before_y else "gt"


def trace_precedence(model):
    """The paper's ``Prec``: for each unordered label pair, the categorical over
    {lt, gt, ilv, excl} induced by the model's TRACE distribution (Assumption 3).

    A coarsening of the stochastic language, so r_Pi = Prec o r_T ties a concurrency to a
    coin flip between its interleavings -- unlike :func:`footprint`."""
    from .traces import trace_distribution

    V = alphabet(model)
    dist = trace_distribution(model)
    out = {}
    for x, y in combinations(V, 2):
        cat = dict.fromkeys(TRACE_PRECEDENCE_KEYS, 0.0)
        for word, p in dist.items():
            cat[_trace_pair_relation(word, x, y)] += p
        out[(x, y)] = cat
    return out


def trace_precedence_angle(m1, m2):
    """Bhattacharyya angle between the two models' pairwise charts r_Pi = Prec o r_T.
    Same form and units as the block-SMD, over the common label pairs."""
    return _pair_chart_angle(trace_precedence(m1), trace_precedence(m2), TRACE_PRECEDENCE_KEYS,
                             alphabet(m1), alphabet(m2), "trace_precedence_angle")


def _pair_chart_angle(p1, p2, keys, V1, V2, name):
    """Shared aggregation for the two pairwise charts: row angles over the common label
    pairs, combined as the block-SMD combines its state rows."""
    if V1 != V2:
        raise ValueError(f"{name} needs a common alphabet (common vertex set).")
    total = 0.0
    per = {}
    for pair in p1:
        ang = _row_angle(p1[pair], p2[pair], keys)
        per[pair] = ang
        total += ang * ang
    return 2.0 * math.sqrt(total), per


# Pre-rename names, kept for the released API.  They are the FOOTPRINT, not the paper's
# r_Pi: `precedence` reads the poset and separates a concurrency from a coin flip.
precedence = footprint
order_angle = footprint_angle
...
# maximally however much structure they share. The fan-out refinement replaces
...
#
# Conventions (each a deliberate choice, stated where it acts):
...
# `block_angle` never refine (the default comparison object). `disc_angle` is the refined
...
# (v1 caveat).
#
# RECURSIVE FAN-OUT (recursive=True, default False -- adoption
...
# content of the canonical-disclosure rule, one level down:
...
def _prime_atoms(prime: Prime, chain_k=2):
    ...
    headline convention). CAVEATS, declared: rung 1 alone is membership-only (two distinct
...
def disc_angle(m1, m2, refine=True, context_depth=1, strict=True, recursive=False, chain_k=2,
               normalize=False):
    """Block-SMD with the fan-out refinement.
    ...
    refine=False       -- atomic blocks throughout (the default comparison).
    ...
    recursive=False    -- recursive fan-out: refined blocks also
    ...
    chain_k=2          -- relational ladder: the rung(s) of prime
    ...
    normalize=False    -- divide by sqrt(|X|) of the pair's union state space: the distance
                          sqrt(|X|) factor, identical to smd(normalize=True). The raw SMD
